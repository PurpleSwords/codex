...
If one platform build fails after other matrix jobs succeed, select that `build_target` and set `reuse_run_id` to the failed workflow run. The repair run downloads the successful native artifacts from that run, builds only the selected platform with a longer timeout, then assembles and publishes a complete six-platform release.
