...
pub(crate) async fn prepare_unified_exec_zsh_fork(
    ...
    let escalation_policy = CoreShellActionProvider {
        ...
        approval_sandbox_permissions: approval_sandbox_permissions(
            // The orchestrator has already authorized the parent request. Match
            // its sandboxed launch when denied reads prevent an override, rather
            // than prompting again for an override we cannot apply. Explicit
            // exec-policy rules are still checked for each intercepted command.
