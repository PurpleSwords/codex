...
async fn backtrack_selection_preserves_selected_prompt_and_requests_branch() {
    ...
    let event = std::iter::from_fn(|| app_event_rx.try_recv().ok())
        ...
        .expect("prompt edit revert should be requested");
    assert_matches!(
        event,
...
#[tokio::test]
async fn backtrack_revert_failure_restores_selected_prompt_snapshot() {
    let (mut app, mut app_event_rx, _op_rx) = make_test_app_with_channels().await;
    ...
    let rendered = lines_to_single_string(&cell.display_lines(/*width*/ 80));
    assert_app_snapshot!(
        "backtrack_revert_failure_restores_selected_prompt",
        rendered
    );
}
...
#[tokio::test]
async fn prompt_edit_reverts_before_selected_prompt_in_same_thread() -> Result<()> {
    let (mut app, mut app_event_rx, _op_rx) = make_test_app_with_channels().await;
    let config = app.chat_widget.config_ref().clone();
    ...
    assert!(matches!(control, AppRunControl::Continue));
    let edited_thread_id = app
        .chat_widget
        .thread_id()
        .expect("prompt edit should keep the current thread");
    assert_eq!(edited_thread_id, source_thread_id);
    ...
    let retained_index = history
        ...
        .expect("edited history should replay the retained prompt");
    let notice_index = history
        .iter()
        .position(|line| line == "• Editing an earlier prompt in this conversation")
        .expect("prompt edit should emit the in-place edit notice");
...
#[tokio::test]
async fn prompt_edit_before_first_prompt_keeps_thread_and_clears_history() -> Result<()> {
    let (mut app, mut app_event_rx, _op_rx) = make_test_app_with_channels().await;
    let config = app.chat_widget.config_ref().clone();
    let source_thread_id = app_test_support::create_fake_rollout(
        config.codex_home.as_path(),
    ...
    let fresh_thread_id = app
        .chat_widget
        .thread_id()
        .expect("first prompt edit should retain the original thread");
    assert_eq!(fresh_thread_id, source_thread_id);
    assert!(
        app_server
            .thread_read(source_thread_id, /*include_turns*/ true)
            .await?
    ...
    assert!(
        history
            .iter()
            .any(|line| { line == "• Editing an earlier prompt in this conversation" })
    );
    assert!(
        !history
            .iter()
