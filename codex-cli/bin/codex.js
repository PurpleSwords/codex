...
delete env.CODEX_GITHUB_REPOSITORY;
if (typeof repositoryUrl === "string") {
  try {
    const repository = new URL(repositoryUrl.replace(/^git\+/, ""));
    const repositoryMatch = repository.pathname.match(
      /^\/([A-Za-z0-9_.-]+)\/([A-Za-z0-9_.-]+?)(?:\.git)?$/,
    );
    if (repository.hostname === "github.com" && repositoryMatch) {
      env.CODEX_GITHUB_REPOSITORY = `${repositoryMatch[1]}/${repositoryMatch[2]}`;
    }
  } catch {
    // Invalid repository metadata must not prevent launching the CLI.
  }
}
